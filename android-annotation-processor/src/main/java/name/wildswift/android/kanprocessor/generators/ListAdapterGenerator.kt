        ...
        return TypeSpec
                ...
                .addFunction(
                        FunSpec.builder("onCreateViewHolder")
                                ...
                                .addCode(CodeBlock.builder()
                                        ...
                                        .addStatement("createdViews = createdViews.filter { it.get() != null } + %T(view)", WeakReference::class.asTypeName())
                                        .also { builder ->
                                            collectionField.elementEvents
                                                    .map { CodeBlock.of("view.${it.resolveListener("${it.name}?.invoke(values[view.tag as? Int ?: 0])")}") }
                ...
                .addFunction(
                        FunSpec.builder("onItemInserted")
                                .addModifiers(KModifier.OVERRIDE)
                                .addParameter(ParameterSpec.builder("index", INT).build())
                                .beginControlFlow("createdViews.forEach")
                                .beginControlFlow("if (it.get()?.tag as? Int ?: -1 >= index)")
                                .addStatement("it.get()?.tag = (it.get()?.tag as? Int)?.plus(1)")
                                .endControlFlow()
                                .endControlFlow()
                ...
                .addFunction(
                        FunSpec.builder("onItemRemoved")
                                ...
                                .beginControlFlow("createdViews.forEach")
                                .beginControlFlow("if (it.get()?.tag as? Int ?: -1 > index)")
                                .addStatement("it.get()?.tag = (it.get()?.tag as? Int)?.minus(1)")
                                .endControlFlow()
                                .beginControlFlow("if (it.get()?.tag as? Int ?: -1 == index)")
                                .addStatement("it.get()?.tag = -1")
                                .endControlFlow()
                                .endControlFlow()
